...
When the button is clicked you should make another call and get the next 10 planets and change 
the table contents with information about the next 10 planets. 
After the next 10 planets are shown the button NEXT 10 should disapear 
and a new button PREVIOUS 10 should appear. 
The previous button should return the first 10 planets in the table 
...
$(document).ready(function () {
  ...
  function showStarWarsPlanets(planets, nextPage, prevPage) {
    ...
    let thName = $(
      '<th style="border: 1px solid black;", width="250px"></th>'
    ).html("Planet name");
    let thPopulation = $(
      '<th style="border: 1px solid black;", width="250px"></th>'
    ).html("Population");
    ...
    for (planet of planets) {
      let tableRow = $("<tr></tr>");
      let tableName = $('<td style="border: 1px solid black;"></td>').html(
        planet.name
      );
      let tablePopulation = $(
        '<td style="border: 1px solid black;"></td>'
      ).html(planet.population);
      let tableClimate = $('<td style="border: 1px solid black;"></td>').html(
        planet.climate
      );
      let tableGravity = $('<td style="border: 1px solid black;"></td>').html(
        planet.gravity
      );
